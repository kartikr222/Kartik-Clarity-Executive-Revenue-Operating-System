...
export default function DiagnosticForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setResult({ error: 'Unable to reach the diagnostic service. Please try again.' });
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {fields.map(([key, label, placeholder]) => (
            <label key={key} className="text-sm text-gray-300">
              {label}
              <input required type="number" min="0" step="any" placeholder={placeholder} value={values[key] || ''} onChange={(e) => setValues((v) => ({ ...v, [key]: e.target.value }))} className="mt-2 w-full rounded-lg border border-white/10 bg-obsidian-bg px-3 py-3 text-white placeholder:text-gray-700" />
            </label>
          ))}
        </div>
        <button type="submit" disabled={loading} className="mt-6 w-full rounded-lg bg-clarity-gold px-5 py-3 font-bold text-black disabled:opacity-50">{loading ? 'Calculating…' : 'Calculate revenue risk →'}</button>
      </form>
      ...
      <div aria-live="polite" className="rounded-2xl border border-white/[0.06] bg-obsidian-surface p-6 sm:p-8">
        ...
        {result?.result && (
          <div>
            ...
            <div className="mt-2 text-6xl font-extrabold">{result.result.score}<span className="text-xl text-gray-500">/100</span></div>
            ...
            <div className="mt-1 text-xl font-bold">{result.result.primaryBottleneck}</div>
            ...
            <ul className="mt-6 space-y-3 text-sm text-gray-400">{result.result.recommendations.map((r) => <li key={r}>• {r}</li>)}</ul>
          </div>
        )}
